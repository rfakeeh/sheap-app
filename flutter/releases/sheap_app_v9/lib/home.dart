...
class _HomePageState extends State<HomePage> {
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            Text(
              "${AppLocalizations.of(context)!.nSos}: $membersSentSOSCount / $membersCount",
              style: TextStyle(
                fontSize: 12,
            ...
            Text(
              "${AppLocalizations.of(context)!.nOutsideGeofence}: $membersOutsideGeofenceCount / $membersCount",
              style: TextStyle(
                fontSize: 12,
            ...
            if (group.isActive)
              _buildActionIcon(
                ...
                () {
                  ...
                  // if OK -> _handleCreatorAlertToGroup(group, receivers, message);
                          ...
                          child: PrimaryButton(
                            onPressed: () async {
                              ...
                              try {
                                ...
                                // get SOS sender info
                                final sosSender = _getSosSender(
                                  widget.user.phoneNumber,
                                );
                                print("sosSender => $sosSender");
                                // determine receivers
                                final receivers =
                                    await _computeSosRequestReceivers(
            ...
            child: PrimaryButton(
              onPressed: () async {
                ...
                try {
                  ...
                  print("sosSender => $sosSender");
                  final receivers = await _computeSosRequestCancelledReceivers(
                    sosSenderPhone: sosSender!['phone'],
                    receivers: List<String>.from(alert.payload['receivers']),
                  );
                  print("receivers => $receivers");
  ...
  Future<Set<String>> _computeSosRequestReceivers({
    required String currentUserPhone,
  }) async {
    final receivers = <String>{};

    // 1) Get all visible members for this user from GroupProvider
    final visibleMembers = context
        .read<GroupProvider>()
        .getAllVisibleMembersFor(currentUserPhone);
  ...
  Future<Set<String>> _computeSosRequestCancelledReceivers({
    ...
    final visibleMembers = context
        .read<GroupProvider>()
        .getAllVisibleMembersFor(sosSenderPhone);
    print("visibleMembers => $visibleMembers");
