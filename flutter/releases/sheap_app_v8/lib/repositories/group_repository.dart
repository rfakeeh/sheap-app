...
class GroupRepository {
  ...
  Stream<List<GroupModel>> getUserGroups(String userPhone) {
    ...
    return groupsRef
        .where(
          Filter.or(
            Filter('memberIds', arrayContains: userPhone),
            Filter('creatorId', isEqualTo: userPhone),
          ),
        )
  ...
  /// Updates only the geofenceConfig field of a group.
  Future<void> updateGeofenceConfig(
    String groupId,
    GeofenceConfig? config,
  ) async {
    final ref = _firestore.collection('groups').doc(groupId);
    await ref.update({'geofenceConfig': config?.toMap()});
  }
}
